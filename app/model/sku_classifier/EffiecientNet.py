...
def construct_model():
    base_model = efn.EfficientNetB0(input_shape = (TARGET_SIZE[0],TARGET_SIZE[1], 3), include_top = False, weights = 'imagenet')
...
def train_model(model_final,X_train,y_train,X_test, y_test):
    ...
    cp_callback = ModelCheckpoint(
        ...
        verbose=1, 
        save_best_only=True)
    eff_history = model_final.fit(x = X_train,y =y_train, validation_data = (X_test,y_test),steps_per_epoch = 30,batch_size = 64, callbacks=[cp_callback], epochs = 20)
    # import pdb
    # pdb.set_trace()
